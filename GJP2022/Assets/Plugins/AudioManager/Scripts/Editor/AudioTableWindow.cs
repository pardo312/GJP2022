...
namespace Jiufen.Audio.Editor
{
    public class AudioTableWindow : EditorWindow
    {
        ...
        private void SetAudioTrack()
        {
            ...
            EditorGUILayout.LabelField("Is Audio track enabled?", GUILayout.Width(AUDIOTABLE_KEY_COLUM_WIDTH * 1.2f));
            m_audioTableScriptable.activateAudioTrack = EditorGUILayout.Toggle(m_audioTableScriptable.activateAudioTrack, GUILayout.Width(AUDIOTABLE_KEY_COLUM_WIDTH));
            GUILayout.EndHorizontal();

            EditorGUILayout.LabelField("", GUILayout.Height(20), GUILayout.ExpandWidth(true));
