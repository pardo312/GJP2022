...
namespace Jiufen.Audio
{
    public class AudioJobsController : MonoBehaviour
    {
        ...
        {
            if (!AudioManager.m_audioTable.ContainsKey(_audioJob.key))
            {
                Debug.LogError("This Key doesn't exist. Please check the audio table");
                yield break;
            }
        ...
        private void RemoveConflictingJobs(string key)
        {
            ...
            {
                AudioTrack currentTrack = (AudioTrack)AudioManager.m_audioTable[job.Key.ToString()];
